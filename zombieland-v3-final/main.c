...
#define LARGURA 800
#define ALTURA 600
enum{triangle, circle, cross, square};
...
{
    if((h->x + h->widthheroi - 8) > i->x + 8 && h->x + 8 < (i->x + i->width - 8) && (h->y + h->heightheroi - 20) > i->y + 20 && h->y - 20 < (i->y + i->height + 20))
    {
...
int main()
{
    ...
    if(!al_install_joystick()){
        mensagem_erro("Erro ao iniciar Joystick");
        return -1;
    }
    ...
    ALLEGRO_JOYSTICK *Controle_1; //Variável que representa o joystick plugado.
    ...
    ALLEGRO_BITMAP *fundo_do_menu = al_load_bitmap("fundoDoMenu.jpg");
    ...
    al_register_event_source(fila_eventos, al_get_joystick_event_source());//Inserindo o joystick como fonte de eventos.
    ...
    while(1){//Loop princiapal (aqui é onde o jogo como um todo ocorre)
        while(esta_no_menu){//Loop do menu
            ...
            al_draw_bitmap(fundo_do_menu,0,0,0);
            ...
            if(evento.type == ALLEGRO_EVENT_DISPLAY_CLOSE){//Se o evento for do tipo fechar a janela.
            ...
            }else if(evento.type == ALLEGRO_EVENT_KEY_DOWN || evento.type == ALLEGRO_EVENT_JOYSTICK_BUTTON_DOWN){//Se o evento detectou que uma tecla foi pressionada
                if(evento.keyboard.keycode == ALLEGRO_KEY_ENTER || evento.joystick.button == cross){
                    puts("ENTER");
                ...
                }else if(evento.joystick.button == circle){
                //Se estiver sobre o label "sair"
                        //A linha abaixo exibe uma janela com duas opções, sim ou não, que retorna o valor 1->sim ou 0->não. E este valor é guardado na variável resposta.
                        int resposta = al_show_native_message_box(janela,"Sair do Jogo","Tem certeza?","Deseja sair do jogo?",NULL,ALLEGRO_MESSAGEBOX_YES_NO);
    ...
    while(esta_no_gameplay){//laço do gameplay.
        ...
        al_draw_bitmap(BG.image,BG.x,BG.y,0);//Desenha o fundo (que será móvel)

        if(BG.x + BG.width < 800){//Se a posição da borda esquerda do fundo for menor que 800
                al_draw_bitmap(BG.image,BG.x + BG.width,BG.y,0);//Desenha outro fundo para preencher este vazio.
                cont++;
                if(cont%100==0)
                {
                    inimigo.x = 800;
